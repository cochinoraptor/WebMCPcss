...
export function rewriteAbsoluteUrls(html: string, target: string, proxyBase: string): string {
  const origin = new URL(target).origin;
  return html.split(origin).join(proxyBase);
}
...
export function createRetroProxy(opts: RetroProxyOptions): http.Server {
  ...
  return http.createServer((req, res) => {
    ...
    if (url === `${PROXY_PREFIX}/webmcp.css`) {
      res.writeHead(200, { 'content-type': 'text/webmcp; charset=utf-8', 'access-control-allow-origin': '*' });
      res.end(opts.css);
      return;
    }
    if (url === `${PROXY_PREFIX}/graph.json`) {
      res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'access-control-allow-origin': '*' });
      res.end(JSON.stringify({ source: opts.target, tools: toolMap.tools, context: toolMap.context }, null, 2));
      return;
    }
    if (url === '/.well-known/webmcp.json') {
      res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'access-control-allow-origin': '*' });
      res.end(JSON.stringify({ version: '1', css: `${PROXY_PREFIX}/webmcp.css`, origin: opts.target, proxy: `webmcpcss@${VERSION}` }, null, 2));
      return;
    }

    // Reenvío al origen.
    const headers: Record<string, string | string[] | undefined> = { ...req.headers, host: target.host, ...opts.headers };
    delete headers['accept-encoding'];
    headers['accept-encoding'] = 'gzip, deflate';
    const upstream = client.request(
      { protocol: target.protocol, hostname: target.hostname, port: target.port || undefined, method: req.method, path: url, headers },
      (up) => {
        const chunks: Buffer[] = [];
        ...
        if (!isHtml) {
          const h = { ...up.headers };
          if (h.location) h.location = String(h.location).replace(target.origin, proxyBase);
        ...
        up.on('end', () => {
          try {
            const raw = decompress(Buffer.concat(chunks), String(up.headers['content-encoding'] ?? ''));
 ...
 */
export async function startRetroProxy(opts: RetroProxyOptions): Promise<{ server: http.Server; url: string }> {
