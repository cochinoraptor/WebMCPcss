...
const ENTITIES: Record<string, string> = {
  nbsp: ' ', amp: '&', lt: '<', gt: '>', quot: '"', apos: "'", euro: '€', copy: '©', reg: '®',
  aacute: 'á', eacute: 'é', iacute: 'í', oacute: 'ó', uacute: 'ú', ntilde: 'ñ', uuml: 'ü',
  Aacute: 'Á', Eacute: 'É', Iacute: 'Í', Oacute: 'Ó', Uacute: 'Ú', Ntilde: 'Ñ', Uuml: 'Ü',
  iexcl: '¡', iquest: '¿', agrave: 'à', egrave: 'è', ccedil: 'ç', ordf: 'ª', ordm: 'º', middot: '·',
};
...
function textOf(html: string): string {
  return decodeEntities(
    html
      ...
      .replace(/<img\b[^>]*\balt=("([^"]*)"|'([^']*)')[^>]*>/gi, (_m, _q, a, b) => ` ${a ?? b ?? ''} `)
      .replace(/<[^>]+>/g, ' ')
      .replace(/\s+/g, ' ')
      .trim(),
  ).slice(0, 80);
...
function isStable(token: string): boolean {
  ...
  if (/^[a-z0-9]{8,}$/i.test(token) && /\d/.test(token) && !/[aeiou]{2}/i.test(token)) return false;
  return true;
}
...
export function proposeSelector(
  ...
  if (attrs.id && isStable(attrs.id)) return { selector: `#${cssEscape(attrs.id)}`, confidence: 0.95 };
  const dataAttr = Object.keys(attrs).find((k) => k.startsWith('data-') && attrs[k] && isStable(attrs[k]));
  if (dataAttr) return { selector: `[${dataAttr}="${cssEscape(attrs[dataAttr])}"]`, confidence: 0.9 };
  if (attrs.name && isStable(attrs.name)) {
    const base = `${t}[name="${cssEscape(attrs.name)}"]`;
    return { selector: context.formSelector ? `${context.formSelector} ${base}` : base, confidence: 0.85 };
  }
  if (attrs['aria-label']) return { selector: `${t}[aria-label="${cssEscape(attrs['aria-label'])}"]`, confidence: 0.8 };
  if (t === 'input' && attrs.value && ['submit', 'button', 'image'].includes((attrs.type ?? '').toLowerCase())) {
    const base = `input[type="${attrs.type.toLowerCase()}"][value="${cssEscape(attrs.value)}"]`;
    return { selector: context.formSelector ? `${context.formSelector} ${base}` : base, confidence: 0.7 };
  }
  if (attrs.title) return { selector: `${t}[title="${cssEscape(attrs.title)}"]`, confidence: 0.6 };
  ...
  if (attrs.href && t === 'a' && /^javascript:/i.test(attrs.href)) {
    ...
    if (fn) return { selector: `a[href^="javascript:${cssEscape(fn)}"]`, confidence: 0.5 };
  }
  if (attrs.class) {
    const stable = attrs.class.split(/\s+/).filter(isStable);
    if (stable.length) {
      const base = `${t}.${stable.slice(0, 2).map(cssEscape).join('.')}`;
      return { selector: context.formSelector ? `${context.formSelector} ${base}` : base, confidence: 0.55 };
    }
  }
  if (attrs.onclick) {
  ...
  if (context.formSelector && (t === 'button' || (t === 'input' && /submit|image/i.test(attrs.type ?? '')))) {
    return { selector: `${context.formSelector} ${t === 'button' ? 'button' : 'input[type="submit"]'}`, confidence: 0.45 };
  }
  if (attrs.alt) return { selector: `${t}[alt="${cssEscape(attrs.alt)}"]`, confidence: 0.55 };
...
export function detectLegacySignals(html: string): LegacySignal[] {
  ...
  push('table-layout', layoutTables, `${layoutTables}/${tables} tablas parecen de maquetación`);
  push('inline-handlers', count(/\son(click|change|submit|load)\s*=/gi), 'manejadores inline on*=');
  push('framesets', count(/<(frameset|frame|iframe)\b/gi), 'frames/iframes');
  push('font-tags', count(/<(font|center|marquee|blink)\b/gi), 'etiquetas presentacionales obsoletas');
  if (!/<!doctype\s+html/i.test(html)) push('no-doctype', 1, 'sin <!DOCTYPE html> (modo quirks o DTD antiguo)');
  if (/<meta[^>]+charset=["']?(iso-8859|windows-125)/i.test(html)) push('quirks-charset', 1, 'charset no UTF-8');
  push('form-without-id', count(/<form\b(?![^>]*\bid=)[^>]*>/gi), 'formularios sin id');
  push('name-only-inputs', count(/<input\b(?![^>]*\bid=)[^>]*\bname=/gi), 'inputs con name pero sin id');
  ...
  push('asp-webforms', count(/__VIEWSTATE|ctl00\$|WebForm_DoPostBack/g), 'ASP.NET WebForms (ids dinámicos, postbacks)');
  if (!/<meta[^>]+name=["']viewport/i.test(html)) push('no-viewport', 1, 'sin meta viewport (no responsive)');
  return signals;
}
...
export function legacyScore(signals: LegacySignal[]): number {
  ...
  let score = 0;
  for (const s of signals) score += weights[s.kind] * Math.min(1, 0.5 + Math.log10(s.count + 1) / 2);
...
function camel(text: string, fallback: string): string {
  ...
  if (ident) {
    ...
    const stripped = ident.replace(/^(txt|btn|frm|ddl|chk|lbl|cmd|lnk|img|rbl|hdn)(?=[A-Z])/, '');
  ...
  const words = decodeEntities(text)
    ...
    .trim()
    .split(/\s+/)
    .filter((w) => w && !/^(el|la|los|las|de|del|un|una|the|a|an|to|of|y|and|en|in)$/i.test(w))
    .slice(0, 3);
 ...
 */
export function extractLegacyElements(html: string): { forms: LegacyForm[]; actions: LegacyElement[] } {
  ...
  while ((fm = formRe.exec(body))) {
    ...
    else if (attrs.name && isStable(attrs.name)) selector = `form[name="${cssEscape(attrs.name)}"]`;
    ...
    while ((cm = ctlRe.exec(inner))) {
      ...
      if (['hidden'].includes(type) || /__VIEWSTATE|__EVENT/i.test(a.name ?? '')) continue;
      const text = textOf(cm[3] ?? '') || a.value || a.placeholder || a.title || '';
      const { selector: sel, confidence } = proposeSelector(tag, a, { formSelector: selector, text, index: i++ });
      const el: LegacyElement = { tag, attrs: a, text, selector: sel, confidence, formIndex: index };
      if (type === 'submit' || type === 'image' || (tag === 'button' && type !== 'button' && type !== 'reset')) {
        if (!submit) submit = el;
  ...
  while ((am = actRe.exec(body))) {
    ...
    const isAction = tag !== 'a' || Boolean(a.onclick) || /^javascript:/i.test(a.href ?? '') || /(login|logout|buscar|search|comprar|buy|cart|carrito|checkout|enviar|submit|contact|registr|sign|download|descargar|pedido|order)/i.test((a.href ?? '') + ' ' + textOf(am[3] ?? '') + ' ' + (a.title ?? ''));
 ...
 */
export function buildRetroToolMap(forms: LegacyForm[], actions: LegacyElement[]): ToolMap {
  ...
  for (const form of forms) {
    ...
    const base = camel(submitText || form.attrs.name || form.attrs.id || form.attrs.action || `form ${form.index + 1}`, `form${form.index + 1}`);
    const name = unique(base);
    const params: ToolSpec['params'] = {};
    form.fields.forEach((f, j) => {
      const pName = camel(f.attrs.name ?? f.attrs.id ?? f.attrs.placeholder ?? `field${j + 1}`, `field${j + 1}`).replace(/[^a-zA-Z0-9]/g, '') || `field${j + 1}`;
      let key = pName;
      let k = 2;
    ...
    const tool: ToolSpec = {
      selector: form.submit?.selector ?? `${form.selector} input[type="submit"], ${form.selector} button`,
      description: submitText ? `Formulario legacy: ${submitText}` : `Envía el formulario ${form.attrs.name ?? form.attrs.id ?? form.index + 1}`,
      params,
      trigger: { event: 'submit', selector: form.selector },
      fingerprint: form.submit ? { tag: form.submit.tag, text: form.submit.text, attrs: pick(form.submit.attrs, ['name', 'value', 'type', 'id']) } : undefined,
  ...
  actions.forEach((a, i) => {
    const label = a.text || a.attrs.title || a.attrs.alt || a.attrs.name || a.attrs.id || `action${i + 1}`;
    const name = unique(camel(label, `action${i + 1}`));
    map.tools[name] = {
      selector: a.selector,
      description: a.attrs.title ? a.attrs.title : a.text ? `Pulsa '${a.text}'` : `Acción ${name}`,
      params: {},
      fingerprint: { tag: a.tag, text: a.text, attrs: pick(a.attrs, ['href', 'name', 'value', 'title', 'alt', 'id']) },
...
export function extractTitle(html: string): string {
  return textOf(/<title[^>]*>([\s\S]*?)<\/title>/i.exec(html)?.[1] ?? '') || 'Sitio legacy';
}
...
export function scanLegacyHtml(html: string, url?: string): RetroScan {
  ...
  const notes: string[] = [];
  const lowConf = Object.entries(toolMap.tools).filter(([, t]) => Number(t.meta?.confidence ?? 1) < 0.5);
  if (lowConf.length) notes.push(`${lowConf.length} herramienta(s) con selector de baja confianza: revisa ${lowConf.map(([n]) => n).join(', ')}.`);
  if (signals.some((s) => s.kind === 'asp-webforms')) notes.push('ASP.NET WebForms: los ids ctl00$… cambian; se han preferido name/value y fingerprints. Considera `webmcpcss repair` tras cada despliegue.');
  if (signals.some((s) => s.kind === 'framesets')) notes.push('Hay frames/iframes: las herramientas dentro de un frame requieren ejecutar con el frame como documento.');
  if (signals.some((s) => s.kind === 'javascript-links')) notes.push('Enlaces javascript: se seleccionan por texto/onclick; añade data-tool si puedes tocar el HTML.');
  return {
    url,
...
export async function fetchHtml(url: string, fetchImpl: typeof fetch = fetch): Promise<string> {
  const res = await fetchImpl(url, {
    headers: { 'user-agent': 'Mozilla/5.0 (compatible; WebMCPcss-retro/1.0)', accept: 'text/html,*/*' },
    redirect: 'follow',
  });
 ...
 */
export async function enhanceRetroWithLlm(scan: RetroScan, client: LlmClient): Promise<number> {
  ...
  let raw: string;
  try {
    raw = await client.complete({ system: LLM_SYSTEM, user: JSON.stringify({ title: scan.title, url: scan.url, tools: summary }), json: true });
  } catch {
    return 0;
  }
  const obj = extractJsonObject(raw);
  const tools = (obj?.tools ?? null) as Record<string, { name?: string; description?: string }> | null;
  if (!tools) return 0;
  let changed = 0;
  ...
  for (const [name, spec] of Object.entries(scan.toolMap.tools)) {
    const s = tools[name];
    let newName = name;
    if (s?.name && /^[a-zA-Z][a-zA-Z0-9]*$/.test(s.name) && !next[s.name]) newName = s.name;
    if (s?.description && typeof s.description === 'string') spec.description = s.description.slice(0, 160);
