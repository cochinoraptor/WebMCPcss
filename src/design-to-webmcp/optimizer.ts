...
export interface OptimizationSuggestion {
  tool: string;
  kind: 'selector' | 'description' | 'confirmation' | 'accessibility' | 'naming' | 'params' | 'format';
...
const SENSITIVE = /pagar|pay|checkout|comprar|buy|purchase|eliminar|delete|borrar|remove|transfer|enviar dinero|cancelar cuenta|unsubscribe|baja/i;
...
export function iaFriendlyScore(map: ToolMap): number {
  ...
  for (const [name, t] of tools) {
    ...
    if (t.description && t.description.length > 8 && !/^(Acción|Pulsa) /.test(t.description)) points += 1;
    if (t.meta?.confirmation || t.confirmation || !SENSITIVE.test(`${name} ${t.description ?? ''}`)) points += 1;
    if (t.meta?.accessibility) points += 0.75;
    if (/^[a-z][a-zA-Z0-9]*$/.test(name) && !/^(action|tool|button|form)\d*$/.test(name)) points += 0.75;
  }
 ...
 */
export function optimizeToolMap(map: ToolMap, options: { apply?: boolean } = {}): OptimizationResult {
  ...
  for (const [name, tool] of Object.entries(out.tools)) {
    ...
    if (frag.level !== 'low' && !/^\[data-tool=/.test(tool.selector)) {
      ...
      suggestions.push({ tool: name, kind: 'selector', severity: frag.level === 'high' ? 'error' : 'warning', message: `Selector ${frag.level === 'high' ? 'frágil' : 'mejorable'} (${frag.reasons[0] ?? frag.level}). Añade data-tool="${toKebab(name)}" al elemento y usa ${after}.`, autofix: true, before: tool.selector, after });
      if (apply) {
    ...
    if (!tool.description || tool.description.length < 8 || /^(Acción|Pulsa) '?[^']*'?$/.test(tool.description)) {
      const after = humanDescription(name, tool.description);
      suggestions.push({ tool: name, kind: 'description', severity: 'warning', message: 'Descripción ausente o genérica: los agentes eligen herramientas por su descripción.', autofix: true, before: tool.description, after });
      if (apply) {
    ...
    if (sensitive && !tool.meta?.confirmation) {
      suggestions.push({ tool: name, kind: 'confirmation', severity: 'error', message: 'Acción sensible sin política de confirmación: añade webmcp-confirmation: "needed".', autofix: true, after: 'needed' });
      if (apply) {
        (tool.meta ??= {}).confirmation = 'needed';
        applied++;
      }
    } else if (!sensitive && !tool.meta?.confirmation && !tool.confirmation) {
      suggestions.push({ tool: name, kind: 'confirmation', severity: 'info', message: 'Declara webmcp-confirmation: "none" para que el agente no tenga que preguntar.', autofix: true, after: 'none' });
      if (apply) {
    ...
    if (!tool.meta?.accessibility) {
      ...
      suggestions.push({ tool: name, kind: 'accessibility', severity: 'warning', message: 'Sin webmcp-accessibility: declara el aria-label esperado del elemento.', autofix: true, after });
      if (apply) {
    ...
    if (!/^[a-z][a-zA-Z0-9]*$/.test(name) || /^(action|tool|button|form)\d*$/.test(name)) {
      suggestions.push({ tool: name, kind: 'naming', severity: 'warning', message: `Nombre '${name}' poco descriptivo o no camelCase; renómbralo según la acción (p. ej. addToCart).`, autofix: false });
    }
    for (const [pName, spec] of Object.entries(tool.params)) {
      if (spec.source === 'value' && !spec.selector && tool.trigger?.event === 'submit') {
        suggestions.push({ tool: name, kind: 'params', severity: 'warning', message: `El parámetro '${pName}' lee el valor del propio botón de envío; indica el campo con value(#selector).`, autofix: false });
      }
    }
  }
  for (const [name, ctx] of Object.entries(out.context)) {
    if (!ctx.format) {
      const after = /price|precio|total|amount|importe/i.test(name) ? 'currency' : /count|cantidad|qty|number|num/i.test(name) ? 'number' : 'text';
      suggestions.push({ tool: name, kind: 'format', severity: 'info', message: `Dato de contexto sin webmcp-format; sugerido "${after}".`, autofix: true, after });
      if (apply) {
        ctx.format = after;
        applied++;
      }
...
function humanDescription(name: string, current?: string): string {
  ...
  const verbs: Record<string, string> = {
    add: 'Añade', remove: 'Elimina', delete: 'Elimina', go: 'Navega a', open: 'Abre', close: 'Cierra', submit: 'Envía', send: 'Envía', search: 'Busca', buy: 'Compra', pay: 'Paga', login: 'Inicia sesión', logout: 'Cierra sesión', register: 'Registra', subscribe: 'Suscribe', apply: 'Aplica', set: 'Establece', select: 'Selecciona', toggle: 'Alterna', save: 'Guarda', cancel: 'Cancela', checkout: 'Finaliza la compra',
  };
